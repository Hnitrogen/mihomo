...
func newClientWithHeaders(srcConn net.Conn, tunnel C.Tunnel, additions []inbound.Addition, headers http.Header) *http.Client {
	return &http.Client{
		Transport: &http.Transport{
			// from http.DefaultTransport
			MaxIdleConns:          100,
			...
			DialContext: func(context context.Context, network, address string) (net.Conn, error) {
				...
				go tunnel.HandleTCPConn(inbound.NewHTTPWithHeaders(dstAddr, srcConn, right, headers, additions...))
